...
First, make a tiny subset of the 
...
First stage filter. Remove snps/indels within 20bp of indels, vars with > 80% missing data, vars with qual < 100. Normally, one would 
...
Before we can filter based on allele-specific things like MAF/MAC/AD, we need to split multiallelics to N biallelic sites for each of the N alternate alleles.
...
Now we can actually do filtering on alleles. Specifically, throw out alleles that: have fewer than a total of 20 reads supporting this allele, a MAF of < 2%, and fewer than 15 allele calls.
...
Finally, join any remaining multiallelics back into actual multialleic sites (this is the inverse of the
...
now tie it all together in one big command, including bcftools to exclude the repeats
